//! ureq collecting variant of the load harness. Standalone — shares no code with
...
//! pool). The response body is fully buffered into memory with `read_to_vec()`, so
//! the whole `.ts` segment is resident at once per worker — the only difference
//! from `ureq_streaming` is this body handling.
//!
//! Run with: `cargo run --release --bin ureq_collecting`
...
/// Upper bound for a buffered body. ureq defaults to 10MB; raise it so the whole
/// segment is collected rather than truncated.
...
async fn main() {
    ...
    println!(
        "[ureq_collecting] Started {WORKERS} workers against {URL}\nReporting memory every {}s. Press Ctrl-C to stop.",
...
/// Performs one synchronous request and buffers the entire body into memory before
/// dropping it. Runs on a blocking thread.
    ...
    loop {
        ...
        match memory_stats::memory_stats() {
            Some(usage) => {
                ...
                println!(
                    "[ureq_collecting] mem: phys {phys_mb:8.2} MB | virt {virt_mb:8.2} MB | requests: {total} total, {per_sec:.1}/s"
                );
            }
            None => {
                println!("[ureq_collecting] mem: <unavailable> | requests: {total} total, {per_sec:.1}/s");
            }
        }
    }
}
